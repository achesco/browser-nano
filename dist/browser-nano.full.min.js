...
},{"./_stream_duplex":20,"_process":14,"buffer":6,"core-util-is":25,"events":10,"inherits":11,"isarray":13,"process-nextick-args":26,"stream":32,"string_decoder/":33,"util":5}],23:[function(require,module,exports){
...
},{"./_stream_duplex":20,"buffer":6,"core-util-is":25,"events":10,"inherits":11,"process-nextick-args":26,"stream":32,"util-deprecate":27}],25:[function(require,module,exports){
(function (Buffer){
...
},{"./lib/_stream_duplex.js":20,"./lib/_stream_passthrough.js":21,"./lib/_stream_readable.js":22,"./lib/_stream_transform.js":23,"./lib/_stream_writable.js":24,"stream":32}],30:[function(require,module,exports){
...
(function (process){
function follow_feed(e,r){var o=new feed.Feed(e);return o.on("error",function(e){return r&&r.call(o,e)}),o.on("change",function(e){return r&&r.call(o,null,e)}),process.nextTick(function(){o.follow()}),o}var feed=require("./lib/feed"),stream=require("./lib/stream");module.exports=follow_feed,module.exports.Feed=feed.Feed,module.exports.Changes=stream.Changes;
}).call(this,require('_process'))
},{"./lib/feed":42,"./lib/stream":44,"_process":14}],42:[function(require,module,exports){
(function (process){
function Feed(e){var t=this;EventEmitter.call(t),e=e||{},t.feed="continuous",t.heartbeat=e.heartbeat||DEFAULT_HEARTBEAT,t.max_retry_seconds=e.max_retry_seconds||DEFAULT_MAX_RETRY_SECONDS,t.inactivity_ms=null,t.initial_retry_delay=e.initial_retry_delay||INITIAL_RETRY_DELAY,t.response_grace_time=e.response_grace_time||RESPONSE_GRACE_TIME,t.headers={},t.request=e.request||{},t.since=0,t.is_paused=!1,t.caught_up=!1,t.retry_delay=t.initial_retry_delay,t.query_params={},"string"==typeof e&&(e={db:e}),Object.keys(e).forEach(function(r){"function"!=typeof t[r]&&(t[r]=e[r])}),t.pending={request:null,activity_at:null}}function destroy_req(e){e&&destroy_response(e.response),e&&"function"==typeof e.destroy&&e.destroy()}function destroy_response(e){e&&("function"==typeof e.abort&&e.abort(),e.connection&&e.connection.destroy())}var lib=require("../lib"),util=require("util"),events=require("events"),request=require("request"),Changes=require("./stream").Changes,querystring=require("querystring"),setTimeout=lib.setTimeout,clearTimeout=lib.clearTimeout,DEFAULT_HEARTBEAT=3e4,HEARTBEAT_TIMEOUT_COEFFICIENT=1.25,DEFAULT_MAX_RETRY_SECONDS=3600,INITIAL_RETRY_DELAY=1e3,RESPONSE_GRACE_TIME=5e3,FEED_PARAMETERS=["since","limit","feed","heartbeat","filter","include_docs","view","style","conflicts","attachments","att_encoding_info"],EventEmitter=events.EventEmitter2||events.EventEmitter;util.inherits(Feed,EventEmitter),Feed.prototype.start=Feed.prototype.follow=function(){var e=this;if(e.db=e.db||e.url||e.uri,delete e.url,delete e.uri,!e.db)throw new Error("Database URL required");if(e.db.match(/\/_db_updates$/)&&(e.is_db_updates=!0),e.is_db_updates&&delete e.since,"continuous"!==e.feed&&"longpoll"!==e.feed)throw new Error('The only valid feed options are "continuous" and "longpoll"');if("number"!=typeof e.heartbeat)throw new Error('Required "heartbeat" value');return e.log=lib.log4js.getLogger(e.db),e.log.setLevel(process.env.follow_log_level||"info"),e.emit("start"),e.confirm()},Feed.prototype.confirm=function(){function e(e,i,a){if(clearTimeout(n),e)return t.die(e);var o;try{o=JSON.parse(a)}catch(s){return t.emit("error",s)}if(!(t.is_db_updates||t.dead||o.db_name&&o.instance_start_time))return t.emit("error",new Error("Bad DB response: "+a));if(t.is_db_updates&&!t.dead&&!o.couchdb)return t.emit("error",new Error("Bad server response: "+a));if(t.is_db_updates||(t.original_db_seq=o.update_seq),t.log.debug("Confirmed "+r+": "+t.db_safe),t.emit("confirm",o),"now"==t.since&&(t.log.debug('Query since "now" is the same as query since -1'),t.since=-1),-1==t.since)t.log.debug("Query since "+t.since+" will start at "+o.update_seq),t.since=o.update_seq;else if(t.since<0){if(isNaN(o.update_seq))return t.emit("error",new Error('DB requires specific id in "since"'));t.log.debug("Query since "+t.since+" will start at "+(o.update_seq+t.since+1)),t.since=o.update_seq+t.since+1}return t.original_db_seq==t.since&&(t.caught_up=!0,t.emit("catchup",o.update_seq)),t.query()}var t=this;t.db_safe=lib.scrub_creds(t.db);var r=t.is_db_updates?"server":"database";t.log.debug("Checking "+r+": "+t.db_safe);var i=3*t.heartbeat,n=setTimeout(function(){return t.die(new Error("Timeout confirming "+r+": "+t.db_safe))},i),a=lib.JP(lib.JS(t.headers));a.accept="application/json";var o=t.is_db_updates?t.db.replace(/\/_db_updates$/,""):t.db,s={uri:o,headers:a};Object.keys(t.request).forEach(function(e){s[e]=t.request[e]}),s=request(s,e),t.emit("confirm_request",s)},Feed.prototype.query=function(){function e(){return r.log.debug("Feed response timed out: "+u),l=!0,r.retry()}function t(e,t,i){if(clearTimeout(_),void 0!==t&&t.body||i)return r.die(new Error("Cannot handle a body in the feed response: "+lib.JS(t.body||i)));if(l)return r.log.debug("Ignoring late response: "+u),destroy_response(t);if(e)return r.log.debug("Request error "+u+": "+e.stack),destroy_response(t),r.retry();if(200!==t.statusCode)return r.log.debug("Bad changes response "+u+": "+t.statusCode),destroy_response(t),r.retry();r.log.debug("Good response: "+u),r.retry_delay=r.initial_retry_delay,r.emit("response",t);var n=new Changes;return n.log=lib.log4js.getLogger("stream "+r.db),n.log.setLevel(r.log.level.levelStr),n.feed=r.feed,c.pipe(n),n.created_at=s,n.id=function(){return u},r.prep(n)}var r=this,i=JSON.parse(JSON.stringify(r.query_params));FEED_PARAMETERS.forEach(function(e){e in r&&(i[e]=r[e])}),"string"!=typeof i.filter&&delete i.filter,"function"!=typeof r.filter||i.include_docs||(r.log.debug("Enabling include_docs for client-side filter"),i.include_docs=!0);var n=100;"longpoll"==i.feed&&(!i.limit||i.limit>n)&&(i.limit=n);var a=r.db+(r.is_db_updates?"":"/_changes")+"?"+querystring.stringify(i);r.headers.accept=r.headers.accept||"application/json";var o={method:"GET",uri:a,headers:r.headers,encoding:"utf-8"};o.changes_query=i,Object.keys(r.request).forEach(function(e){o[e]=r.request[e]});var s=new Date,d=lib.JDUP(s),u=process.env.follow_debug?d.match(/\.(\d\d\d)Z$/)[1]:d;r.log.debug("Feed query "+u+": "+lib.scrub_creds(a));var c=request(o);c.on("response",function(e){r.log.debug("Remove feed from agent pool: "+u),c.req.socket.emit("agentRemove"),t(null,e,e.body)}),c.on("error",t);var _=setTimeout(e,r.heartbeat+r.response_grace_time),l=!1;return r.emit("query",c)},Feed.prototype.prep=function(e){function t(t){function i(){if(r.pending.request===e)return a.apply(r,arguments);if(!e.created_at)return r.die(new Error("Received data from unknown request"));var i=(new Date-e.created_at)/1e3,n="[no req]";r.pending.request&&(n=(r.pending.request.created_at-e.created_at)/1e3);var o=": "+e.id()+" to_req="+n+"s, to_now="+i+"s";return"end"==t||"data"==t||"heartbeat"==t?(r.log.debug('Old "'+t+'": '+e.id()),destroy_req(e)):void r.log.warn('Old "'+t+'"'+o)}var n="on_couch_"+t,a=r[n];return i}var r=this,i=new Date;return r.pending.request=e,r.pending.activity_at=i,r.pending.wait_timer=null,r.is_paused?r.pause():r.resume(),r.change_at=i,r.inactivity_ms&&(clearTimeout(r.inactivity_timer),r.inactivity_timer=setTimeout(function(){r.on_inactivity()},r.inactivity_ms)),e.on("heartbeat",t("heartbeat")),e.on("error",t("error")),e.on("data",t("data")),e.on("end",t("end")),r.wait()},Feed.prototype.wait=function(){var e=this;if(e.emit("wait"),e.pending.wait_timer)return e.die(new Error("wait() called but there is already a wait_timer: "+e.pending.wait_timer));var t=e.heartbeat*HEARTBEAT_TIMEOUT_COEFFICIENT,r=e.pending.request&&e.pending.request.id(),i="Req "+r+" timeout="+t;e.inactivity_ms&&(i+=", inactivity="+e.inactivity_ms),i+=": "+e.db_safe,e.log.debug(i),e.pending.wait_timer=setTimeout(function(){e.on_timeout()},t)},Feed.prototype.got_activity=function(){var e=this;e.dead||(clearTimeout(e.pending.wait_timer),e.pending.wait_timer=null,e.pending.activity_at=new Date)},Feed.prototype.pause=function(){var e=this,t=e.is_paused;e.is_paused=!0,e.pending&&e.pending.request&&e.pending.request.pause?e.pending.request.pause():e.log.warn("No pending request to pause"),t||e.emit("pause")},Feed.prototype.resume=function(){var e=this,t=e.is_paused;e.is_paused=!1,t&&e.emit("resume"),e.pending&&e.pending.request&&e.pending.request.resume?e.pending.request.resume():e.log.warn("No pending request to resume")},Feed.prototype.on_couch_heartbeat=function(){var e=this;return e.got_activity(),e.dead?e.log.debug("Skip heartbeat processing for dead feed"):(e.emit("heartbeat"),e.dead?e.log.debug("No wait: heartbeat listener stopped this feed"):void e.wait())},Feed.prototype.on_couch_data=function(e){var t=this;if(t.log.debug("Data from "+t.pending.request.id()),t.got_activity(),t.dead)return t.log.debug("Skip data processing for dead feed");if(e=JSON.parse(e),!t.is_db_updates&&"last_seq"in e){t.log.warn("Stopping upon receiving a final message: "+JSON.stringify(e));var r=new Error("Database deleted after change: "+e.last_seq);return r.deleted=!0,r.last_seq=e.last_seq,t.die(r)}return t.is_db_updates||e.seq?(t.on_change(e),t.dead?t.log.debug("No wait: change listener stopped this feed"):void t.wait()):t.die(new Error("Change has no .seq field: "+JSON.stringify(e)))},Feed.prototype.on_timeout=function(){var e=this;if(e.dead)return e.log.debug("No timeout: change listener stopped this feed");e.log.debug("Timeout");var t=new Date,r=t-e.pending.activity_at;e.emit("timeout",{elapsed_ms:r,heartbeat:e.heartbeat,id:e.pending.request.id()}),destroy_req(e.pending.request),e.retry()},Feed.prototype.retry=function(){var e=this;clearTimeout(e.pending.wait_timer),e.pending.wait_timer=null,e.log.debug("Retry since="+e.since+" after "+e.retry_delay+"ms "),e.emit("retry",{since:e.since,after:e.retry_delay,db:e.db_safe}),e.retry_timer=setTimeout(function(){e.query()},e.retry_delay);var t=1e3*e.max_retry_seconds;e.retry_delay*=2,e.retry_delay>t&&(e.retry_delay=t)},Feed.prototype.on_couch_end=function(){var e=this;return e.log.debug("Changes feed ended "+e.pending.request.id()),e.pending.request=null,e.retry()},Feed.prototype.on_couch_error=function(e){var t=this;return t.log.debug("Changes query eror: "+lib.JS(e.stack)),t.retry()},Feed.prototype.stop=function(e){var t=this;t.log.debug("Stop"),t.die(),t.emit("stop",e)},Feed.prototype.die=function(e){var t=this;e&&t.log.fatal("Fatal error: "+e.stack),t.dead=!0,clearTimeout(t.retry_timer),clearTimeout(t.inactivity_timer),clearTimeout(t.pending.wait_timer),t.inactivity_timer=null,t.pending.wait_timer=null;var r=t.pending.request;t.pending.request=null,r&&(t.log.debug("Destroying req "+r.id()),destroy_req(r)),e&&t.emit("error",e)},Feed.prototype.on_change=function(e){var t=this;if(!t.is_db_updates&&!e.seq)return t.die(new Error("No seq value in change: "+lib.JS(e)));if(!t.is_db_updates&&e.seq==t.since)return t.log.debug("Bad seq value "+e.seq+" since="+t.since),destroy_req(t.pending.request);if("function"!=typeof t.filter)return t.on_good_change(e);var r,i=lib.JDUP({query:t.pending.request.changes_query});if(t.is_db_updates){if(!e.db_name||!e.type)return t.die(new Error("Internal _db_updates filter needs .db_name and .type in change ",e));r=[e.db_name,e.type,i]}else{if(!e.doc)return t.die(new Error("Internal filter needs .doc in change "+e.seq));var n=lib.JDUP(e.doc);r=[n,i]}var a=!1;try{a=t.filter.apply(null,r)}catch(o){t.log.debug("Filter error",o)}return(a=a&&!0||!1)?(t.log.debug("Builtin filter PASS for change: "+e.seq),t.on_good_change(e)):(t.log.debug("Builtin filter FAIL for change: "+e.seq),void t.check_for_catchup(e.seq))},Feed.prototype.on_good_change=function(e){var t=this;return t.inactivity_ms&&!t.inactivity_timer?t.die(new Error("Cannot find inactivity timer during change")):(clearTimeout(t.inactivity_timer),t.inactivity_timer=null,t.inactivity_ms&&(t.inactivity_timer=setTimeout(function(){t.on_inactivity()},t.inactivity_ms)),t.change_at=new Date,t.is_db_updates||(t.since=e.seq),t.emit("change",e),void t.check_for_catchup(e.seq))},Feed.prototype.check_for_catchup=function(e){var t=this;t.is_db_updates||t.caught_up||e<t.original_db_seq||(t.caught_up=!0,t.emit("catchup",e))},Feed.prototype.on_inactivity=function(){var e=this,t=new Date,r=t-e.change_at,i=r/1e3;return e.log.debug("Req "+e.pending.request.id()+" made no changes for "+i+"s"),e.restart()},Feed.prototype.restart=function(){var e=this;e.emit("restart"),e.stop(),e.dead=!1,e.start()},module.exports={Feed:Feed};
}).call(this,require('_process'))
},{"../lib":43,"./stream":44,"_process":14,"events":10,"querystring":18,"request":3,"util":36}],43:[function(require,module,exports){
function getLogger(e){return{trace:noop,debug:debug("follow:"+e+":debug"),info:debug("follow:"+e+":info"),warn:debug("follow:"+e+":warn"),error:debug("follow:"+e+":error"),fatal:debug("follow:"+e+":fatal"),level:{level:0,levelStr:"noop"},setLevel:noop}}function noop(){}exports.scrub_creds=function(e){return e.replace(/^(https?:\/\/)[^:]+:[^@]+@(.*)$/,"$1$2")},exports.JP=JSON.parse,exports.JS=JSON.stringify,exports.JDUP=function(e){return JSON.parse(JSON.stringify(e))};var timeouts={setTimeout:setTimeout,clearTimeout:clearTimeout};exports.setTimeout=function(){return timeouts.setTimeout.apply(this,arguments)},exports.clearTimeout=function(){return timeouts.clearTimeout.apply(this,arguments)},exports.timeouts=function(e,t){timeouts.setTimeout=e,timeouts.clearTimeout=t};var debug=require("debug");exports.log4js={getLogger:getLogger};
},{"debug":45}],44:[function(require,module,exports){
(function (process,Buffer){
function Changes(e){var r=this;stream.call(r),r.readable=!0,r.writable=!0,r.headers={},r.statusCode=null,e=e||{},r.feed=e.feed||null,r.encoding=e.encoding||"utf8",r.log=e.log,r.log||(r.log=lib.log4js.getLogger("change_stream"),r.log.setLevel(DEFS.log_level)),r.is_sending=!0,r.is_ending=!1,r.is_dead=!1,r.source=null,r.expect=null,r.buf=null,r.changes=[],r.on("pipe",function(e){if(r.source){var t=new Error("Already have a pipe source");t.source=r.source,r.error(t)}else r.source=e})}var lib=require("../lib"),util=require("util"),stream=require("stream"),request=require("request"),setTimeout=lib.setTimeout,clearTimeout=lib.clearTimeout,DEFS={longpoll_header:'{"results":[',log_level:process.env.follow_log_level||"info"};module.exports={Changes:Changes},util.inherits(Changes,stream),Changes.prototype.setHeader=function(e,r){var t=this;t.headers[e]=r},Changes.prototype.setEncoding=function(e){var r=this;r.encoding=e},Changes.prototype.pause=function(){var e=this;e.is_sending=!1,e.source&&e.source.pause&&e.source.pause()},Changes.prototype.resume=function(){var e=this;e.is_sending=!0,e.source&&e.source.resume&&e.source.resume(),e.emit_changes()},Changes.prototype.write=function(e,r){var t=this;return e=t.normalize_data(e,r),"string"==typeof e?"longpoll"===t.feed?t.write_longpoll(e):"continuous"===t.feed?t.write_continuous(e):void 0:void 0},Changes.prototype.write_longpoll=function(e){var r=this;return null===r.buf&&(r.buf=[]),r.buf.push(e),!0},Changes.prototype.write_continuous=function(e){var r,t,n,o=this,s=(o.buf||"")+e;for(o.log.debug("write: "+util.inspect({data:e,buf:s}));(r=s.indexOf("\n"))>=0;){if(t=s.substr(0,r),s=s.substr(r+1),o.log.debug("JSON: "+util.inspect(t)),""===t);else{if("{"!==t[0])return o.error(new Error("Non-object JSON data: "+t));try{n=JSON.parse(t)}catch(i){return o.error(i)}o.log.debug("Object: "+util.inspect(n)),t=JSON.stringify(n)}o.changes.push(t)}var u=o.is_sending;return o.buf=s,o.emit_changes(),u},Changes.prototype.end=function(e,r){var t=this;if(t.is_ending=!0,t.writable=!1,t.write(e,r),"longpoll"===t.feed){var n=[DEFS.longpoll_header].concat(t.buf).join("");try{n=JSON.parse(n)||{}}catch(o){return t.error(o)}return Array.isArray(n.results)?0!==t.changes.length?t.error(new Error("Changes are already queued: "+JSON.stringify(t.changes))):(t.changes=n.results.map(function(e){return JSON.stringify(e)}),t.emit_changes()):t.error(new Error('No "results" field in feed'))}"continuous"===t.feed&&""!==t.buf&&t.log.debug('Unprocessed data after "end" called: '+util.inspect(t.buf))},Changes.prototype.emit_changes=function(){for(var e=this;e.is_sending&&e.changes.length>0;){var r=e.changes.shift();""===r?(e.log.debug("emit: heartbeat"),e.emit("heartbeat")):(e.log.debug("emit: data"),e.emit("data",r))}e.is_sending&&e.is_ending&&0===e.changes.length&&(e.is_ending=!1,e.readable=!1,e.log.debug("emit: end"),e.emit("end"))},Changes.prototype.destroy=function(){var e=this;return e.log.debug("destroy"),e.is_dead=!0,e.is_ending=!1,e.is_sending=!1,e.source&&"function"==typeof e.source.abort?e.source.abort():(e.source&&"function"==typeof e.source.destroy&&e.source.destroy(),void(e.source&&e.source.__isRequestRequest&&e.source.response&&"function"==typeof e.source.response.destroy&&e.source.response.destroy()))},Changes.prototype.destroySoon=function(){throw new Error("not implemented")},Changes.prototype.normalize_data=function(e,r){var t=this;if(e instanceof Buffer?e=e.toString(r):"undefined"==typeof e&&"undefined"==typeof r&&(e=""),"string"!=typeof e)return t.error(new Error("Not a string or Buffer: "+util.inspect(e)));if("continuous"!==t.feed&&"longpoll"!==t.feed)return t.error(new Error('Must set .feed to "continuous" or "longpoll" before writing data'));null===t.expect&&(t.expect="longpoll"==t.feed?DEFS.longpoll_header:"");var n=e.substr(0,t.expect.length);e=e.substr(n.length);var o=t.expect.substr(0,n.length),s=t.expect.substr(o.length);return n!==o?t.error(new Error("Prefix not expected "+util.inspect(o)+": "+util.inspect(n))):(t.expect=s,e)},Changes.prototype.error=function(e){var r=this;return r.readable=!1,r.writable=!1,r.emit("error",e),!1};
